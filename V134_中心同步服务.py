# -*- coding: utf-8 -*-
"""聚宝盆客户信息管理系统 中心同步服务 v134 正式修复版
v134（409/删除确认闭环正式修复）：
  · 冲突响应增加稳定 code/action/retryable；危险删除挑战继续绑定 Source/Epoch/Seq/Hash。
v132（多机共用服务端）：
...
import V134_shared_db as shared_db
...
APP_VERSION='134'; SCHEMA_VERSION=2
...
def claim_lease(payload):
 ...
 with CONTACT_WRITE_LOCK:
  ...
  try:
   ...
   if r and int(r['expires_at'] or 0)>t and str(r['installation_id'])!=inst:c.rollback();return {'ok':False,'conflict':True,'error':'conflict','code':'SOURCE_LEASE_HELD','reason':'source-lease-held','retryable':False,'action':'wait_or_use_unique_source','leaseExpiresAt':int(r['expires_at'])},409
...
def sync_snapshot(payload,require_v133=False):
 ...
 with CONTACT_WRITE_LOCK:
  ...
  try:
   ...
   if require_v133:
    if payload.get('protocolVersion')!=133 or not _valid_lease(cur,payload,sid):c.rollback();return {'ok':False,'conflict':True,'error':'conflict','code':'STALE_WRITER_FENCED','reason':'stale-writer-fenced','retryable':False,'action':'reacquire_lease'},409
   elif lease_exists:
    c.rollback();return {'ok':False,'conflict':True,'error':'conflict','code':'PROTOCOL_UPGRADE_REQUIRED','reason':'protocol-upgrade-required','retryable':False,'action':'upgrade_client'},409
   oldbatch=cur.execute('SELECT * FROM sync_batches WHERE source_id=? AND client_epoch=? AND batch_id=?',(sid,epoch,bid)).fetchone()
   if oldbatch:
    if oldbatch['request_hash']!=req:c.rollback();return {'ok':False,'conflict':True,'error':'conflict','code':'BATCH_CONTENT_MISMATCH','reason':'batch-content-mismatch','retryable':False,'action':'rebuild_batch'},409
   ...
   if head and head['active_client_epoch']!=epoch:
    c.rollback();return {'ok':False,'conflict':True,'error':'conflict','code':'CLIENT_EPOCH_MISMATCH','reason':'client-epoch-mismatch','retryable':False,'action':'stop_and_reconcile_epoch','activeClientEpoch':head['active_client_epoch'],'serverAckSeq':int(head['ack_seq'])},409
   ack=int(head['ack_seq']) if head else 0;server_snap=str(head['last_snapshot_hash'] or '') if head else ''
   if seq>ack+1:c.rollback();return {'ok':False,'conflict':True,'error':'conflict','code':'SNAPSHOT_SEQUENCE_GAP','reason':'snapshot-sequence-gap','retryable':False,'action':'reconcile_sequence','serverAckSeq':ack,'expectedSnapshotSeq':ack+1},409
   if seq<ack:
   ...
   if seq==ack:
    if snap!=server_snap:c.rollback();return {'ok':False,'conflict':True,'error':'conflict','code':'SNAPSHOT_SEQUENCE_CONFLICT','reason':'snapshot-sequence-conflict','retryable':False,'action':'reconcile_sequence','serverAckSeq':ack,'expectedSnapshotSeq':ack+1,'serverSnapshotHash':server_snap},409
   ...
   if destructive:
    ...
    if not confirmed:
     ...
     return {'ok':False,'conflict':True,'error':'conflict','code':'DELETE_CONFIRMATION_REQUIRED','reason':'destructive-snapshot-requires-confirmation','retryable':False,'action':'confirm_delete','beforeCount':len(oldhash),'afterCount':len(clean),'deleteCount':len(remove),'deleteRatio':delete_ratio,'snapshotSeq':seq,'snapshotHash':snap,'confirmationToken':token,'confirmationExpiresInMs':300000},409
...
def capabilities():return {'snapshotSyncV2':True,'leaseFenceV1':True,'destructiveGuardV1':True,'structuredConflictV1':True,'atomicAdminMutationV1':True,'snapshotSyncV1':True,'syncStatusV1':True,'factsRevisionV1':True,'serverAggregateV1':False,'epochFenceV1':True,'remoteOverrideV1':True,'commandChannelV1':True}
...
def main():
 ...
 print('v134：危险删除确认闭环 + 409 分类熔断；继续兼容 v133 同步协议')
...
# 1. 补全危险删除确认闭环：确认令牌绑定 Source、Epoch 与 Snapshot Hash，且一次性消费。
# 2. 409 响应提供稳定的 code、reason、retryable、action 和冲突诊断字段。
# 3. 小规模来源仅在删除数量达到安全门槛后才按比例触发危险删除保护。
# 4. 保留 Lease、Epoch、Snapshot 序号、Batch 幂等及事务原子性安全围栏。
# 5. 对外版本升级为 V134；/api/v133 与 protocolVersion=133 作为兼容协议继续保留。
# ===============================================================================
