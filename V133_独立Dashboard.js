// ==UserScript==
// @name         聚宝盆客户信息管理系统 v133 独立Dashboard
// @namespace    wa-remark-helper
// @version      133
// @description  v133 安全同步版：Lease 写入围栏、原子管理操作、分区命令游标与可靠 ACK；兼容旧服务端。多机共用服务端——①同步服务地址可在菜单中配置（默认 http://127.0.0.1:8765），多台电脑/浏览器指向同一中心服务；②新增下行指令通道：他源编辑/删除会下发 contact.update / contact.delete 指令，来源浏览器轮询执行后本地数据与服务端收敛，覆盖层自动清理；③支持可选访问令牌（服务端 JUBO_TOKEN）。
...
(function () {
  ...
  var STANDALONE_DASHBOARD_MODE = (location.port === "8765" && location.pathname === "/dashboard");
...
function getPanelDisplayData() {
    ...
    async function v129UploadSnapshot(sourceId,sourceName,snapshot) {
      ...
      var res=await localSyncRequest('POST',useV133?'/api/v133/sync/snapshot':'/api/v129/sync/snapshot',requestBody);
      state=v129ReadProtocolState(sourceId);
      if (!res.ok) {
