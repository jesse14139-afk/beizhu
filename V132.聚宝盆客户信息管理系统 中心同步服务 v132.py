...
"""
import hashlib, json, os, shutil, sqlite3, threading, time, traceback
...
HOST=(os.environ.get('JUBO_HOST') or '0.0.0.0').strip() or '0.0.0.0'
try:PORT=int(os.environ.get('JUBO_PORT') or 8765)
except Exception:PORT=8765
APP_VERSION='132'; SCHEMA_VERSION=1
...
def poll_commands(q):
 sid=(q.get('sourceId') or [''])[0].strip();after=int((q.get('afterId') or ['0'])[0] or 0);epoch=(q.get('clientEpoch') or [''])[0].strip()
 if not sid:return {'ok':False,'error':'missing sourceId'}
 c=conn();cur=c.cursor()
 try:
  ...
  return {'ok':True,'sourceId':sid,'count':len(items),'items':items}
 except Exception as e:return {'ok':False,'error':str(e),'trace':traceback.format_exc()}
 finally:c.close()
def ack_commands(payload):
 sid=str(payload.get('sourceId') or '').strip();ids=payload.get('commandIds');status=str(payload.get('status') or 'done').strip()[:20] or 'done';note=str(payload.get('note') or '').strip()[:500];t=now_ms()
 if not sid:return {'ok':False,'error':'missing sourceId'}
 if not isinstance(ids,list) or not ids:return {'ok':False,'error':'commandIds must be non-empty array'}
 ids=[int(x) for x in ids if str(x).strip().lstrip('-').isdigit()]
 if not ids:return {'ok':False,'error':'invalid commandIds'}
 if status not in ('done','error'):status='done'
 with CONTACT_WRITE_LOCK:
  c=conn();cur=c.cursor()
  try:
   cur.execute('BEGIN IMMEDIATE')
   placeholders=','.join(['?']*len(ids))
   cur.execute('UPDATE commands SET status=?,acked_at=?,ack_note=?,updated_at=? WHERE source_id=? AND id IN ('+placeholders+')',(status,t,note,t,sid)+tuple(ids))
   cleared=[]
   for cid in ids:
    row=cur.execute('SELECT kind,payload_json FROM commands WHERE source_id=? AND id=?',(sid,cid)).fetchone()
    if not row:continue
    try:pl=json.loads(row['payload_json'] or '{}')
    except Exception:pl={}
    target=str(pl.get('contactId') or pl.get('id') or '')
    if not target:continue
    if row['kind']=='contact.update' and _auto_clear_override(cur,sid,target):cleared.append(target)
    elif row['kind']=='contact.delete' and _auto_clear_tombstone(cur,sid,target):cleared.append(target)
   c.commit();return {'ok':True,'sourceId':sid,'updated':len(ids),'convergedCleared':cleared}
  except Exception as e:c.rollback();return {'ok':False,'error':str(e),'trace':traceback.format_exc()}
  finally:c.close()
...
def capabilities():return {'snapshotSyncV1':True,'syncStatusV1':True,'factsRevisionV1':True,'serverAggregateV1':False,'epochFenceV1':True,'remoteOverrideV1':True,'commandChannelV1':True}
...
class H(BaseHTTPRequestHandler):
 ...
 def _auth_ok(self):
  ...
  return (self.headers.get('X-Auth-Token') or '')==AUTH_TOKEN
 ...
 def do_GET(self):
  try:
   ...
   if p.path=='/api/health':return self.sendj({'ok':True,'service':'wa-remark-local-sync','version':APP_VERSION,'sharedConfig':True,'time':now_ms(),'dbPath':str(DB_PATH),'dbType':shared_db.db_type(),'auth':bool(AUTH_TOKEN),'capabilities':capabilities()})
   ...
   if p.path=='/api/commands/poll':return self.sendj(poll_commands(q))
...
def main():
 ...
 print('v132：多机共用（共享库抽象 + 下行指令通道 commandChannelV1）；v131 覆盖层保留；v126~v130 接口全部兼容')
