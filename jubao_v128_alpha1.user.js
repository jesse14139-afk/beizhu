// ==UserScript==
// @name         聚宝盆 v128-alpha1
// @namespace    wa-remark-helper
// @version      128.0.1-alpha1
// @description  v128-alpha1：持久化上传状态、跨标签页脏状态通知、上传竞态保护、失败退避及休眠恢复补传；保留 v126 全部功能。
...
(function () {
  ...
  var V128_SYNC_VERSION = "128.0.1-alpha1";
...
function getPanelDisplayData() {
    ...
    async function localSyncUploadOnly() {
      ...
      var shouldSettlePersistentState = !!(pendingAtStart.dirty || pendingAtStart.changeSeq > pendingAtStart.ackSeq);
      try {
        var h = await localSyncHealthCheck({ silent:true });
        if (!h.ok) {
          if (shouldSettlePersistentState) v128FinishUploadFailure(uploadSeq,h.error || ('HTTP '+(h.status || '连接失败')));
          return h;
        }
        var up = await localSyncUploadAll();
        LOCAL_SYNC_LAST_STATUS.message = up.ok ? "本源上传完成" : "本源上传失败";
        if (shouldSettlePersistentState) {
          if (up && up.ok) v128FinishUploadSuccess(uploadSeq);
          else v128FinishUploadFailure(uploadSeq,up || '上传失败');
        }
        return up;
      } catch(e) {
        if (shouldSettlePersistentState) v128FinishUploadFailure(uploadSeq,e);
        throw e;
      } finally {
        v117ReleaseUploadLease();
    ...
    function v128NormalizeUploadState(value) {
      ...
      state.changeSeq=Math.max(0,Number(state.changeSeq)||0);
      state.ackSeq=Math.max(0,Math.min(state.changeSeq,Number(state.ackSeq)||0));
      state.retryCount=Math.max(0,Number(state.retryCount)||0);
      state.nextRetryAt=Math.max(0,Number(state.nextRetryAt)||0);
    ...
    function v128WriteUploadState(state) {
      ...
      try { GM_setValue(V128_UPLOAD_STATE_KEY,state); } catch(e) { console.error('[v128] 保存上传状态失败',e); }
      return state;
    }
    ...
    function v128MarkUploadDirty() {
      var state=v128ReadUploadState();
      state.dirty=true;
      state.changeSeq=Math.max(state.changeSeq,state.ackSeq)+1;
      state.changedAt=Date.now();
      state.changedByTab=String(V117_TAB_ID || '');
      state.lastError='';
      state.nextRetryAt=0;
      v128WriteUploadState(state);
      return state.changeSeq;
    }
    ...
    function v128ScheduleRetry(state, reason) {
      ...
      var wait=Math.max(0,Number(state.nextRetryAt||0)-Date.now());
      v128ClearRetryTimer();
    ...
    function v128FinishUploadFailure(startSeq,error) {
      var state=v128ReadUploadState(), now=Date.now();
      state.dirty=true;
      state.uploadFinishedAt=now;
    ...
    function v128InstallUploadStateListener() {
      ...
      v117RegisterListener(V128_UPLOAD_STATE_KEY,function(n,o,v,remote){
        ...
        if (V117_IS_LEADER && (state.dirty || state.changeSeq > state.ackSeq)) {
          v128CheckPendingUpload(remote ? 'follower-dirty' : 'shared-dirty');
        }
      });
    }
    ...
    async function drainLocalSyncUploadQueue(reason) {
      ...
      var uploadLock=await v117AcquireUploadLease();
      if (!uploadLock) {
        clearTimeout(LOCAL_SYNC_UPLOAD_TIMER);
      ...
      var uploadSeq=state.changeSeq;
      state.lastAttemptAt=Date.now();
      state.uploadStartedAt=state.lastAttemptAt;
      state.lastError='';
      v128WriteUploadState(state);
      try {
        ...
        var h=await localSyncHealthCheck({ silent:true });
        if (!h.ok) {
          var failedHealth=v128FinishUploadFailure(uploadSeq,h.error || ('HTTP '+(h.status || '连接失败')));
          v892SetRuntime({ upload:'retry', uploadError:'同步服务已断开，将自动重试：'+failedHealth.lastError });
          return;
        }
        var result=await localSyncUploadAll();
        if (!result || !result.ok) {
          var failedUpload=v128FinishUploadFailure(uploadSeq,result || '未知错误');
          v892SetRuntime({ upload:'retry', uploadError:'上传失败，将自动重试：'+failedUpload.lastError });
          return;
        }
        var done=v128FinishUploadSuccess(uploadSeq);
        v892SetRuntime({
          upload:done.dirty ? 'pending' : 'ok',
          lastUploadAt:Date.now(), uploadError:''
        });
        await v892RefreshCounts();
      } catch(e) {
        var failedException=v128FinishUploadFailure(uploadSeq,e);
        ...
        LOCAL_SYNC_LAST_AUTO_AT=Date.now();
        if (V117_IS_LEADER && v128HasPendingUpload() && isLocalSyncRealtimeUploadEnabled()) {
          v128CheckPendingUpload('dirty-after-upload');
        }
      }
    }
    ...
    async function runLocalSyncAggregateOnce(renderAllFn) {
      ...
      LOCAL_SYNC_AGGREGATE_RUNNING = true;
      try {
        v892SetRuntime({ summary:'busy', summaryError:'' });
        var result = await localSyncRefreshAggregate();
        if (result && result.ok) {
          var sources = LOCAL_SYNC_LAST_STATUS.sources || [];
          var stats = LOCAL_SYNC_LAST_STATUS.stats || {};
          v892SetRuntime({
            summary:'ok', lastSummaryAt:Date.now(), totalSources:sources.length,
            totalContacts:stats.totalContacts || 0, summaryError:''
          });
          v117PublishAggregateCache();
          if (typeof renderAllFn === 'function') renderAllFn();
          return result;
        }
        v892SetRuntime({
          summary:aggregateHasCacheV112() ? 'stale' : 'error',
          summaryError:'汇总失败：' + ((result && (result.error || result.stage)) || '未知错误')
        });
        return result || { ok:false, error:'未知错误' };
      } catch(e) {
        v892SetRuntime({
          summary:aggregateHasCacheV112() ? 'stale' : 'error',
          summaryError:'汇总异常：' + ((e && e.message) || e)
        });
        return { ok:false, error:(e && e.message) || String(e) };
      } finally {
        LOCAL_SYNC_AGGREGATE_RUNNING = false;
      }
    }
    ...
    function localSyncDiagText() {
      ...
      return [
        "v120 WhatsApp 精确身份修复 / 跨窗口同步诊断",
        "当前标签页: " + v117TabLabel(),
        "是否主监控: " + (V117_IS_LEADER ? "是" : "否"),
        "主监控租约: " + JSON.stringify(v117ReadLeader() || null),
        "实时上传本源: " + (isLocalSyncRealtimeUploadEnabled() ? "开启" : "关闭"),
        "v128上传状态: " + JSON.stringify(v128ReadUploadState()),
    ...
    async function localSyncUploadAll() {
      var sourceId = getLocalSyncSourceId();
      var sourceName = getLocalSyncSourceName();
      // v121.1：上传前迁移同源幽灵键，并先清除 SQLite 中的旧包装 ID。
      var ghostRepair = v121RepairSameSourceSyncGhosts();
      if (ghostRepair.deletedIds.length) {
        var ghostDelete = await localSyncBatchDelete(ghostRepair.deletedIds);
        if (!ghostDelete || !ghostDelete.ok) console.warn('[v121.1] 服务端幽灵记录删除失败，将在强制重建时清除', ghostRepair.deletedIds, ghostDelete);
      }
      var data = loadData();
      var items = [];
      ...
      var res = await localSyncRequest("POST", "/api/contacts/batch-upsert", {
        sourceId: sourceId, sourceName: sourceName, schemaVersion: 105, presentationVersion: 1, recordModel: "leaf-facts-only", uploadedAt: Date.now(), items: items
      });
      LOCAL_SYNC_LAST_STATUS.ok = !!res.ok;
    ...
    function v117OwnsLeaderLease() {
      ...
      return !!(x && x.leaderId === V117_TAB_ID && Number(x.expiresAt || 0) > Date.now());
    }
    function v117LeaderValid(x) { return !!(x && x.leaderId && Number(x.expiresAt || 0) > Date.now()); }
    ...
    function v117BackgroundRender() {
      try {
        if (managePanelWindow && !managePanelWindow.closed && typeof managePanelWindow.__waRefresh === 'function') managePanelWindow.__waRefresh();
      } catch(e) {}
      try {
    ...
    async function v117AcquireUploadLease() {
      var key=v117UploadLeaseKey(), now=Date.now(), cur=v117ParseShared(GM_getValue(key,''), null);
      if (cur && cur.owner !== V117_TAB_ID && Number(cur.expiresAt || 0)>now) return false;
      GM_setValue(key, JSON.stringify({owner:V117_TAB_ID,sourceId:getLocalSyncSourceId(),channel:v117Channel(),expiresAt:now+15000}));
      await new Promise(function(resolve){ setTimeout(resolve,60+Math.floor(Math.random()*80)); });
      var verify=v117ParseShared(GM_getValue(key,''),null);
      return !!(verify && verify.owner===V117_TAB_ID && Number(verify.expiresAt||0)>Date.now());
    }
    function v117ReleaseUploadLease() {
      var key=v117UploadLeaseKey(), cur=v117ParseShared(GM_getValue(key,''),null);
      if (cur && cur.owner===V117_TAB_ID) GM_setValue(key, JSON.stringify({owner:'',expiresAt:0,releasedAt:Date.now()}));
    }
    ...
    function v117Wake() {
      if (V117_IS_LEADER) { v117RenewLeader(); v128CheckPendingUpload('lifecycle-wake'); return; }
