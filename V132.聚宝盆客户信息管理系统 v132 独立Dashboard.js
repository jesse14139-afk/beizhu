// ==UserScript==
// @name         聚宝盆客户信息管理系统 v132 独立Dashboard
// @namespace    wa-remark-helper
// @version      132
// @description  v132：多机共用服务端——①同步服务地址可在菜单中配置（默认 http://127.0.0.1:8765），多台电脑/浏览器指向同一中心服务；②新增下行指令通道：他源编辑/删除会下发 contact.update / contact.delete 指令，来源浏览器轮询执行后本地数据与服务端收敛，覆盖层自动清理；③支持可选访问令牌（服务端 JUBO_TOKEN）。
...
function getPanelDisplayData() {
    ...
    function remoteOverrideSaveField(channelId, key, value) {
      ...
      localSyncRequest('POST', '/api/contacts/override', { sourceId: parts.sid, contactId: parts.cid, updatedBy: getLocalSyncSourceName(), fields: fields }).then(function (res) {
        if (res && res.ok) {
          // v132：下发 contact.update 指令，让来源浏览器本地收敛（最终一致，覆盖层随后自动清理）
          v132EnqueueCommand(parts.sid, 'contact.update', { contactId: parts.cid, fields: fields });
    ...
    function remoteOverrideDelete(channelId) {
      ...
      var parts = v99ParseRemoteKey(channelId);
      if (!parts) return false;
      localSyncRequest('POST', '/api/contacts/override-delete', { sourceId: parts.sid, contactId: parts.cid, updatedBy: getLocalSyncSourceName() }).then(function (res) {
        if (res && res.ok) {
          // v132：下发 contact.delete 指令，让来源浏览器本地删除（下次快照移除行，墓碑随后自动清理）
          v132EnqueueCommand(parts.sid, 'contact.delete', { contactId: parts.cid });
    ...
    function v132GetLastCommandId() { try { return parseInt(GM_getValue('wa_local_sync_last_command_id', '0'), 10) || 0; } catch (e) { return 0; } }
    function v132SetLastCommandId(id) { try { GM_setValue('wa_local_sync_last_command_id', String(id || 0)); } catch (e) {} }
    ...
    function v132PollCommands() {
      ...
      var sid = '';
      try { sid = getLocalSyncSourceId(); } catch (e) {}
      var state = null;
      try { state = v129ReadProtocolState(sid); } catch (e) {}
      var epoch = (state && state.clientEpoch) || '';
      return localSyncRequest('GET', '/api/commands/poll?sourceId=' + encodeURIComponent(sid) + '&afterId=' + v132GetLastCommandId() + '&clientEpoch=' + encodeURIComponent(epoch))
        .then(function (res) {
          ...
          items.forEach(function (cmd) {
            chain = chain.then(function () {
              return v132ExecuteCommand(cmd).then(function (result) {
                ...
                return localSyncRequest('POST', '/api/commands/ack', { sourceId: sid, commandIds: [cmd.id], status: status, note: note }).then(function () {
                  v132SetLastCommandId(Math.max(v132GetLastCommandId(), Number(cmd.id) || 0));
                });
              });
            });
          });
          return chain.then(function () {
            try { if (isLocalSyncAggregateEnabled()) localSyncRefreshAggregate(); } catch (e) {}
          });
        })
        .catch(function () {})
