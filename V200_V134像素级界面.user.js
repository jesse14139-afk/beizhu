// ==UserScript==
// @name         V200.1兼容版 - 聚宝盆客户信息管理系统 v134 独立Dashboard
// @namespace    wa-remark-helper
// @version      200.1.0
// @description  v134 正式修复版：危险删除确认闭环、409 分类熔断、Lease/Epoch 围栏；兼容 v133 同步协议。
// @match        https://web.whatsapp.com/*
// @match        http://127.0.0.1:8765/dashboard*
// @match        http://localhost:8765/dashboard*
// @match        https://www.instagram.com/direct/*
// @match        https://www.messenger.com/*
// @match        https://www.facebook.com/messages/*
// @match        https://web.telegram.org/k/*
// 多机部署：远程打开 Dashboard 时，把下面的示例改成你的服务器地址（match 模式不写端口，会自动匹配任意端口，
// 例如 http://192.168.1.10:8765/dashboard 只需写 http://192.168.1.10/dashboard*），然后删掉本行与示例行开头的"//"：
// @match        http://192.168.1.10/dashboard*
...
(function () {
  ...
  // v132：远程 Dashboard 模式（多机）。任何主机、只要端口 8765 + 路径 /dashboard 就挂载管理面板；
  // 需要在 Tampermonkey 中为本脚本的 @match 加入服务器地址（match 模式不写端口，任意端口均可匹配）。
  var STANDALONE_DASHBOARD_MODE = (location.pathname === "/dashboard");
  ...
  var SYSTEM_VERSION_V124 = (function () {
    try {
      ...
      return String(value || "130").replace(/^v/i, "").trim() || "130";
    } catch (e) { return "130"; }
  })();
  ...
  // v132：多机共用。服务地址不再硬编码——可在 Tampermonkey 菜单“设置同步服务地址”中
  // 指向中心服务端（局域网 IP 或公网地址），多台电脑/浏览器共用同一服务与共享库。
  var LOCAL_SYNC_URL = "http://127.0.0.1:8765";
  try { var __v132InitServerUrl = String(GM_getValue('wa_local_sync_server_url', '') || '').trim().replace(/\/+$/, ''); if (__v132InitServerUrl) LOCAL_SYNC_URL = __v132InitServerUrl; } catch (e) {}
  function getLocalSyncServerUrl() {
    try { var u = String(GM_getValue('wa_local_sync_server_url', '') || '').trim().replace(/\/+$/, ''); if (u) return u; } catch (e) {}
    return LOCAL_SYNC_URL;
  }
  function setLocalSyncServerUrl(u) {
    u = String(u || '').trim().replace(/\/+$/, '');
    try { GM_setValue('wa_local_sync_server_url', u); } catch (e) {}
    LOCAL_SYNC_URL = u || 'http://127.0.0.1:8765';
    return LOCAL_SYNC_URL;
  }
  function getLocalSyncAuthHeaders() {
    var headers = { 'Content-Type': 'application/json' };
    try { var t = String(GM_getValue('wa_local_sync_token', '') || '').trim(); if (t) headers['X-Auth-Token'] = t; } catch (e) {}
    return headers;
  }
  ...
  var V128_SYNC_VERSION = "129.0.0-alpha1.1";
...
function sharedConfigRequestV125(method, path, body) {
  return new Promise(function(resolve) {
    try {
      GM_xmlhttpRequest({method:method,url:getLocalSyncServerUrl()+path,headers:getLocalSyncAuthHeaders(),data:body?JSON.stringify(body):undefined,timeout:8000,
...
async function initSharedConfigV125() {
  ...
  function upsertFieldSmart(channelId, baseInfo, key, value) {
    ...
    // v131：他源汇总记录走服务端覆盖层，不写本地 GM 业务库、不进本源快照。
    if (isRemoteSyncWrappedId(channelId)) return remoteOverrideSaveField(channelId, key, value);
    var data = loadData();
  ...
  function toggleStageSmart(channelId, stageKey, checked) {
    if (isProtectedIgThreadIdV1217(channelId)) return;
    // v131：他源阶段变更走覆盖层，基于当前汇总值增量修改。
    if (isRemoteSyncWrappedId(channelId)) return remoteOverrideSaveStages(channelId, stageKey, checked);
  ...
  function upsertTagSmart(channelId, baseInfo, value) {
    ...
    // v131：他源标签走覆盖层。
    if (isRemoteSyncWrappedId(channelId)) return remoteOverrideSaveField(channelId, 'tag', value);
  ...
  function upsertTag2Smart(channelId, tags2Array) {
    if (isProtectedIgThreadIdV1217(channelId)) return;
    // v131：他源画像走覆盖层。
    if (isRemoteSyncWrappedId(channelId)) return remoteOverrideSaveField(channelId, 'tag2', (Array.isArray(tags2Array) ? tags2Array : []).filter(function (x) { return String(x || '').trim(); }));
    var data = loadData();
...
html += '.table-container{background:var(--card);border:1px solid var(--border);border-radius:8px;overflow:auto;max-height:72vh;position:relative;}\n';
    ...
    html += '<label id="remoteEditToggleLabel" style="display:inline-flex;align-items:center;gap:4px;font-size:12px;color:var(--text,#111b21);white-space:nowrap;cursor:pointer;" title="开启后，他源汇总数据可在本面板直接编辑/删除（保存到服务端覆盖层，不被来源浏览器快照覆盖）；关闭则恢复他源只读。"><input type="checkbox" id="remoteEditToggle" style="width:14px;height:14px;cursor:pointer;"> 他源可编辑</label>\n';
...
html += '<div class="sub-table-header">\n' +
      ...
      return new Promise(function(resolve){
        ...
        try {
          requestHandle = GM_xmlhttpRequest({
            method: method,
            url: getLocalSyncServerUrl() + path,
            headers: getLocalSyncAuthHeaders(),
    ...
    // v131：他源可编辑开关（默认开启）。开启后他源行可编辑/删除，改动写入服务端覆盖层。
    function isRemoteEditableEnabled() {
      try { return getSettings().remoteEditableV131 !== false; } catch (e) { return true; }
    }
    function setRemoteEditableEnabled(v) {
      try { setSettings({ remoteEditableV131: !!v }); } catch (e) {}
    }
    function guardRemoteSyncEdit(row) {
      if (!isRemoteSyncRecord(row)) return false;
      if (isRemoteEditableEnabled()) return false;
      try {
...
function getPanelDisplayData() {
    ...
    async function localSyncUploadOnly() {
      ...
      try {
        ...
        if (!h.ok) {
          v133FinishUploadByPolicy(uploadSeq,h,taskToken);
          return h;
        }
        ...
        else v133FinishUploadByPolicy(uploadSeq,up || '上传失败',taskToken);
        return up;
      } catch(e) {
    ...
    async function localSyncRefreshAggregate() {
      ...
      var pair = await Promise.all([localSyncFetchSourcesAndStats(), localSyncFetchAllContacts()]);
      var meta = pair[0];
      var contacts = pair[1];
    ...
    function v128DefaultUploadState() {
      return {
        ...
        blocked:false, blockCode:'', blockReason:'', blockedAt:0,
    ...
    function v128NormalizeUploadState(value) {
      ...
      ['changedAt','uploadStartedAt','uploadFinishedAt','uploadLeaseUntil','lastAttemptAt','lastSuccessAt','nextRetryAt','lastWatchdogAt','repairedAt','blockedAt'].forEach(function(k){ state[k]=v128SafeNumber(state[k]); });
      ['changedByTab','changeToken','uploadTaskId','uploadByTab','lastError','repairReason','blockCode','blockReason'].forEach(function(k){ state[k]=String(state[k]||''); });
    ...
    function v128EventLog(event, detail, level) {
      ...
      try { console[method]('[v134]['+event+']',safe); } catch(e) {}
    }
    ...
    function v128MarkUploadDirty() {
      ...
      for (var attempt=0; attempt<3; attempt++) {
        ...
        state.lastError=''; state.nextRetryAt=0; state.blocked=false; state.blockCode=''; state.blockReason=''; state.blockedAt=0;
    ...
    function v128FinishUploadSuccess(startSeq,token) {
      ...
      state.blocked=false; state.blockCode=''; state.blockReason=''; state.blockedAt=0;
    ...
    function v133UploadFailureDetail(error) {
      var data=(error && error.data && typeof error.data==='object') ? error.data : {};
      var status=Number((error && error.status)||0);
      var code=String(data.code || data.reason || (error && error.code) || '').trim();
      var reason=String(data.message || data.reason || data.error || (error && (error.error||error.message||error.statusText)) || (status ? ('HTTP '+status) : '上传失败'));
      var retryable=!!(error && (error.hardTimeout || status===0 || status===408 || status===425 || status===429 || status>=500));
      if (data.retryable===true) retryable=true;
      if (data.retryable===false || status===400 || status===401 || status===403 || status===404 || status===409 || status===413 || status===422) retryable=false;
      return {status:status,code:code||('HTTP_'+status),reason:reason,retryable:retryable,data:data};
    }
    function v128FinishUploadBlocked(startSeq,error,token) {
      if (token && !v128TaskIsCurrent(token)) return v128ReadUploadState();
      var state=v128ReadUploadState(), now=Date.now(), detail=v133UploadFailureDetail(error);
      if (token && state.uploadTaskId && state.uploadTaskId!==token) return state;
      state.dirty=true; state.uploadFinishedAt=now; state.failureCount=Math.max(0,state.failureCount)+1;
      state.lastError=detail.reason; state.retryCount=0; state.nextRetryAt=0;
      state.blocked=true; state.blockCode=detail.code; state.blockReason=detail.reason; state.blockedAt=now;
      state.uploadTaskId=''; state.uploadByTab=''; state.uploadLeaseUntil=0; state.uploadStartedAt=0;
      state=v128WriteUploadState(state); v128ClearRetryTimer();
      v128EventLog('upload-blocked',{taskToken:token,status:detail.status,code:detail.code,reason:detail.reason,response:detail.data,failureCount:state.failureCount},'error');
      return state;
    }
    function v133FinishUploadByPolicy(startSeq,error,token) {
      var detail=v133UploadFailureDetail(error);
      return detail.retryable ? {state:v128FinishUploadFailure(startSeq,error,token),blocked:false,detail:detail}
        : {state:v128FinishUploadBlocked(startSeq,error,token),blocked:true,detail:detail};
    }
    ...
    async function drainLocalSyncUploadQueue(reason) {
      ...
      try {
        ...
        if (!h.ok) {
          var healthPolicy=v133FinishUploadByPolicy(uploadSeq,h,taskToken), failedHealth=healthPolicy.state;
          v892SetRuntime({ upload:healthPolicy.blocked?'blocked':'retry', uploadError:(healthPolicy.blocked?'同步已暂停：':'同步服务已断开，将自动重试：')+failedHealth.lastError }); return;
        }
        var result=await localSyncUploadAll();
        if (!v128TaskIsCurrent(taskToken)) return;
        if (!result || !result.ok) {
          var uploadPolicy=v133FinishUploadByPolicy(uploadSeq,result||'未知错误',taskToken), failedUpload=uploadPolicy.state;
          v892SetRuntime({ upload:uploadPolicy.blocked?'blocked':'retry', uploadError:(uploadPolicy.blocked?'上传已暂停：':'上传失败，将自动重试：')+failedUpload.lastError }); return;
        }
        ...
        if (v128TaskIsCurrent(taskToken)) {
          var exceptionPolicy=v133FinishUploadByPolicy(uploadSeq,e,taskToken), failedException=exceptionPolicy.state;
          v892SetRuntime({ upload:exceptionPolicy.blocked?'blocked':'retry', uploadError:(exceptionPolicy.blocked?'上传已暂停：':'上传异常，将自动重试：')+failedException.lastError });
        }
      } finally {
    ...
    function startLocalSyncHeartbeat(renderAllFn) {
      ...
      async function heartbeat() {
        ...
        try {
          ...
          // v132：下行指令轮询（Leader 标签页执行一次；与心跳同频，约 5 秒）
          try { v132PollCommands(); } catch (e) {}
    ...
    function localSyncDiagText() {
      ...
      return [
        "聚宝盆 "+getDisplayVersionV124()+" / 跨窗口同步诊断",
        JSON.stringify(snapshot,null,2),
        "",
        "实时汇总显示: " + (isLocalSyncAggregateEnabled() ? "开启" : "关闭"),
        "URL: " + getLocalSyncServerUrl(),
    ...
    // ==================== v131 他源覆盖层（方案 B） ====================
    // 他源行的编辑/删除不写本地 GM 业务库、不进本源快照，而是写入服务端
    // contact_overrides 表；/api/contacts 读取时由服务端合并覆盖并过滤墓碑，
    // 本源浏览器快照重传不会冲掉管理端改动。
    function isRemoteSyncWrappedId(value) {
      return v121IsSyncWrappedId(value) && !v121IsSameSourceSyncGhostId(value);
    }
    function v99ParseRemoteKey(value) {
      var s = String(value == null ? '' : value);
      if (s.indexOf('sync::') !== 0) return null;
      var rest = s.slice('sync::'.length);
      var i = rest.indexOf('::');
      if (i <= 0) return null;
      var sid = rest.slice(0, i), cid = rest.slice(i + 2);
      if (!sid || !cid) return null;
      return { sid: sid, cid: cid };
    }
    function remoteOverrideToast(msg) {
      try { showCopyToast(msg); } catch (e) {}
    }
    function remoteOverrideFailText(res) {
      try { return ((res && res.data && (res.data.error || res.data.reason)) || (res && res.error) || (res && res.status) || '连接失败'); } catch (e) { return '连接失败'; }
    }
    function remoteOverrideRefresh() {
      localSyncRefreshAggregate().then(function () { try { renderAll(); } catch (e) {} }).catch(function () { try { renderAll(); } catch (e) {} });
    }
    function remoteOverrideSaveField(channelId, key, value) {
      if (!isRemoteEditableEnabled()) { remoteOverrideToast('⚠️ 他源可编辑已关闭'); return false; }
      var parts = v99ParseRemoteKey(channelId);
      if (!parts) return false;
      var fields = {}; fields[key] = value;
      // v133：覆盖层与下行命令由服务端在同一事务中提交，禁止客户端拆成两次写入。
      localSyncRequest('POST', '/api/v133/admin/contacts/update', { sourceId: parts.sid, contactId: parts.cid, updatedBy: getLocalSyncSourceName(), fields: fields }).then(function (res) {
        if (res && res.ok && res.data && res.data.ok === true) remoteOverrideRefresh();
        else remoteOverrideToast('⚠️ 他源保存失败：' + remoteOverrideFailText(res));
      });
      return true;
    }
    function remoteOverrideSaveStages(channelId, stageKey, checked) {
      if (!isRemoteEditableEnabled()) { remoteOverrideToast('⚠️ 他源可编辑已关闭'); return false; }
      var row = null;
      try {
        var rows = getAllRows();
        for (var i = 0; i < rows.length; i++) { if (rows[i].__id === channelId) { row = rows[i]; break; } }
      } catch (e) {}
      var arr = Array.isArray(row && row.stages) ? row.stages.slice() : [];
      var idx = arr.indexOf(stageKey);
      if (checked) { if (idx < 0) arr.push(stageKey); }
      else if (idx >= 0) { arr.splice(idx, 1); }
      return remoteOverrideSaveField(channelId, 'stages', arr);
    }
    function remoteOverrideDelete(channelId) {
      if (!isRemoteEditableEnabled()) { remoteOverrideToast('⚠️ 他源可编辑已关闭'); return false; }
      var parts = v99ParseRemoteKey(channelId);
      if (!parts) return false;
      var basePath = '/api/v200/contacts/' + encodeURIComponent(parts.cid);
      var deviceId = getLocalSyncSourceName() || 'v134-admin';
      remoteOverrideToast('正在生成安全删除预览…');
      sharedConfigRequestV125('POST', basePath + '/delete-preview', null).then(function (previewRes) {
        var preview = previewRes && previewRes.data;
        if (!previewRes || !previewRes.ok || !preview || preview.ok !== true || !preview.confirmationId || !preview.impact) {
          remoteOverrideToast('⚠️ 删除预览失败：' + remoteOverrideFailText(previewRes));
          return;
        }
        var impact = preview.impact || {};
        if (String(impact.contactId || '') !== String(parts.cid) || String(impact.sourceId || '') !== String(parts.sid)) {
          remoteOverrideToast('⚠️ 删除预览身份校验失败，已停止操作');
          return;
        }
        var expiresText = preview.expiresAt ? new Date(Number(preview.expiresAt)).toLocaleString() : '未知';
        var message = '确认删除以下联系人？\n\n' +
          '姓名：' + String(impact.displayName || '未命名') + '\n' +
          '电话：' + String(impact.phone || '无') + '\n' +
          '来源：' + String(impact.sourceId || '') + '\n' +
          '当前版本：' + String(impact.version == null ? '' : impact.version) + '\n' +
          '关联标签：' + String(impact.tagCount || 0) + ' 个\n' +
          '关联客户：' + String(impact.customerCount || 0) + ' 个\n' +
          '确认凭证有效至：' + expiresText + '\n\n' +
          '删除后联系人将进入回收站，可由管理员恢复。';
        var accepted = false;
        try {
          var w = (typeof managePanelWindow !== 'undefined' && managePanelWindow && !managePanelWindow.closed) ? managePanelWindow : window;
          accepted = !!w.confirm(message);
        } catch (e) { try { accepted = !!window.confirm(message); } catch (_e) {} }
        if (!accepted) { remoteOverrideToast('已取消删除，数据未发生变化'); return; }
        sharedConfigRequestV125('DELETE', basePath, {
          baseVersion: Number(impact.version),
          confirmationId: String(preview.confirmationId),
          reason: 'V134 管理后台人工确认删除',
          deviceId: deviceId
        }).then(function (deleteRes) {
          if (deleteRes && deleteRes.ok && deleteRes.data && deleteRes.data.ok === true) {
            try { delete state.selected[channelId]; } catch (e) {}
            remoteOverrideToast('联系人已移入回收站');
            remoteOverrideRefresh();
          } else {
            var code = deleteRes && deleteRes.data && deleteRes.data.code;
            var hint = code === 'VERSION_CONFLICT' ? '联系人已被其他设备修改，请刷新后重试' :
              code === 'INVALID_CONFIRMATION' ? '删除凭证已过期、失效或已使用，请重新操作' : remoteOverrideFailText(deleteRes);
            remoteOverrideToast('⚠️ 他源删除失败：' + hint);
          }
        });
      });
      return true;
    }
    function appendRemoteEditableBadge(doc, parent, row) {
      var badge = doc.createElement('span');
      badge.className = 'wa-remote-readonly-badge wa-v106-source-badge';
      badge.textContent = getRemoteSyncSourceLabel(row) + (isRemoteEditableEnabled() ? '·可编辑' : '');
      badge.style.setProperty('--wa-v106-source-color', getSourceBadgeColorV106(row));
      badge.title = isRemoteEditableEnabled()
        ? '这是来自其他源【' + getRemoteSyncSourceLabel(row) + '】的汇总数据；已开启“他源可编辑”，改动保存在服务端覆盖层，来源浏览器快照不会覆盖。'
        : getRemoteSyncReadonlyTitle(row);
      parent.appendChild(badge);
    }
    ...
    // ==================== v132 下行指令通道（多机共用） ====================
    // 服务端维护每 source 的指令队列；本源浏览器（Leader 标签页）轮询拉取并执行，
    // 执行后回执 ack。用途：
    //   contact.update         —— 管理端在 Dashboard 编辑他源字段后，让来源浏览器本地收敛
    //   contact.delete         —— 管理端删除他源记录后，让来源浏览器本地删除（下次快照移除行）
    //   resync                 —— 全量重传（快照重建）
    //   shared-config.refresh  —— 刷新公共业务配置
    // 覆盖层/墓碑在数据收敛后由服务端自动清理（contact_overrides 不再永久叠加）。
    var V132_COMMAND_POLL_RUNNING = false;
    function isRemoteDownlinkEnabled() { try { return getSettings().remoteDownlinkV132 !== false; } catch (e) { return true; } }
    function setRemoteDownlinkEnabled(v) { try { var s = getSettings(); s.remoteDownlinkV132 = !!v; setSettings(s); } catch (e) {} }
    // v133：命令游标必须按服务端、来源、Epoch 三维隔离，切换服务器时不得复用旧游标。
    function v133ServerFingerprint() {
      var normalized=String(getLocalSyncServerUrl()||'').trim().replace(/\/+$/,'').toLowerCase();
      try { return String(v117Hash(normalized)); } catch(e) { return normalized.replace(/[^a-z0-9]/g,'_').slice(0,80); }
    }
    function v133CommandCursorKey(sid,epoch) {
      return 'wa_sync_v133_command_cursor::'+v133ServerFingerprint()+'::'+encodeURIComponent(String(sid||''))+'::'+encodeURIComponent(String(epoch||''));
    }
    function v132GetLastCommandId(sid,epoch) { try { return parseInt(GM_getValue(v133CommandCursorKey(sid,epoch), '0'), 10) || 0; } catch (e) { return 0; } }
    function v132SetLastCommandId(sid,epoch,id) { try { GM_setValue(v133CommandCursorKey(sid,epoch), String(id || 0)); } catch (e) {} }
    function v133GetInstallationId() {
      var key='wa_sync_v133_installation_id', value='';
      try { value=String(GM_getValue(key,'')||''); } catch(e) {}
      if (!value) { value=v129NewId('installation'); try { GM_setValue(key,value); } catch(e2) {} }
      return value;
    }
    function v132EnqueueCommand(sid, kind, payload) {
      if (!isRemoteDownlinkEnabled()) return;
      try {
        localSyncRequest('POST', '/api/commands/create', { sourceId: sid, kind: kind, payload: payload || {}, createdBy: getLocalSyncSourceName() || '' }).then(function () {});
      } catch (e) {}
    }
    function v132ApplyContactUpdate(payload) {
      var cid = String((payload && (payload.contactId || payload.id)) || '').trim();
      var fields = (payload && payload.fields) || {};
      if (!cid) return { ok: false, note: 'missing contactId' };
      if (isMainKey(cid) || isProtectedIgThreadIdV1217(cid)) return { ok: false, note: 'protected record' };
      var data = loadData() || {};
      var existed = Object.prototype.hasOwnProperty.call(data, cid);
      var merged = Object.assign({}, data[cid] || {}, fields);
      merged.updatedAt = Date.now();
      if (!existed && !hasMeaningfulUserData(merged)) { return { ok: true, note: 'empty-update-skipped' }; }
      data[cid] = merged;
      saveData(data);
      return { ok: true, note: existed ? 'updated' : 'created' };
    }
    function v132ApplyContactDelete(payload) {
      var cid = String((payload && (payload.contactId || payload.id)) || '').trim();
      if (!cid) return { ok: false, note: 'missing contactId' };
      if (isMainKey(cid) || isProtectedIgThreadIdV1217(cid)) return { ok: false, note: 'protected record' };
      var data = loadData() || {};
      if (Object.prototype.hasOwnProperty.call(data, cid)) { delete data[cid]; saveData(data); return { ok: true, note: 'deleted' }; }
      return { ok: true, note: 'not-exists' };
    }
    function v132ExecuteCommand(cmd) {
      var kind = String((cmd && cmd.kind) || '');
      var payload = (cmd && cmd.payload) || {};
      try {
        if (kind === 'contact.update') return Promise.resolve(v132ApplyContactUpdate(payload));
        if (kind === 'contact.delete') return Promise.resolve(v132ApplyContactDelete(payload));
        if (kind === 'resync') {
          return localSyncUploadAll().then(function (r) { return { ok: !!(r && r.ok), note: (r && (r.error || r.message)) || '' }; });
        }
        if (kind === 'shared-config.refresh') {
          return Promise.resolve().then(function () { try { pollSharedConfigV125(); } catch (e) {} return { ok: true, note: '' }; });
        }
        return Promise.resolve({ ok: false, note: 'unknown-kind:' + kind });
      } catch (e) { return Promise.resolve({ ok: false, note: String((e && e.message) || e) }); }
    }
    function v132PollCommands() {
      if (V132_COMMAND_POLL_RUNNING) return Promise.resolve();
      V132_COMMAND_POLL_RUNNING = true;
      var sid = ''; try { sid = getLocalSyncSourceId(); } catch (e) {}
      var protocolState = null; try { protocolState = v129ReadProtocolState(sid); } catch (e2) {}
      var epoch = (protocolState && protocolState.clientEpoch) || '';
      var afterId = v132GetLastCommandId(sid,epoch);
      return localSyncRequest('GET', '/api/commands/poll?sourceId=' + encodeURIComponent(sid) + '&afterId=' + afterId + '&clientEpoch=' + encodeURIComponent(epoch))
        .then(function (res) {
          var items = res && res.ok && res.data && Array.isArray(res.data.items) ? res.data.items : [];
          var chain = Promise.resolve();
          items.forEach(function (cmd) {
            chain = chain.then(function () {
              return v132ExecuteCommand(cmd).then(function (result) {
                var note = (result && result.note) || '';
                var status = (result && result.ok) ? 'done' : 'error';
                return localSyncRequest('POST', '/api/commands/ack', {
                  sourceId:sid, clientEpoch:epoch, commandIds:[cmd.id], status:status, note:note
                }).then(function (ack) {
                  // 只有明确收到业务层 ok 才推进；失败时抛出并停止后续命令，保证下轮重试。
                  if (!(ack && ack.ok && ack.data && ack.data.ok === true)) throw new Error('command-ack-not-confirmed');
                  v132SetLastCommandId(sid,epoch,Math.max(v132GetLastCommandId(sid,epoch),Number(cmd.id)||0));
                });
              });
            });
          });
          return chain.then(function () { try { if (isLocalSyncAggregateEnabled()) localSyncRefreshAggregate(); } catch (e) {} });
        })
        .catch(function () {})
        .then(function () { V132_COMMAND_POLL_RUNNING = false; });
    }

    function v129DefaultProtocolState(sourceId) {
      return {
        schemaVersion:2, sourceId:String(sourceId || ''), clientEpoch:'', nextSnapshotSeq:1,
        ...
        protocol:'unknown', leaseGeneration:0, leaseToken:'', leaseExpiresAt:0, installationId:'', updatedAt:Date.now()
      };
    }
    ...
    async function v129FetchSyncStatus(sourceId,clientEpoch) {
      var suffix='?sourceId='+encodeURIComponent(sourceId)+(clientEpoch?'&clientEpoch='+encodeURIComponent(clientEpoch):'');
      var res=await localSyncRequest('GET','/api/v133/sync/status'+suffix);
      if (res && res.status===404) { res=await localSyncRequest('GET','/api/v129/sync/status'+suffix); res.apiVersion=129; }
      else if (res) res.apiVersion=133;
      return res;
    }
    function v129ApplyServerStatus(state,data) {
      ...
      state.lastError=''; state.protocol=(data.capabilities&&data.capabilities.snapshotSyncV2)?'v133':'v129'; v129WriteProtocolState(state);
    ...
    async function v133ClaimLease(sourceId,state) {
      var installationId=v133GetInstallationId();
      var res=await localSyncRequest('POST','/api/v133/sync/lease/claim',{sourceId:sourceId,installationId:installationId,clientEpoch:state.clientEpoch});
      if (!(res && res.ok && res.data && res.data.ok===true && res.data.leaseToken && Number(res.data.leaseGeneration)>0)) return res || {ok:false,error:'lease-claim-failed'};
      state.installationId=installationId; state.leaseGeneration=Number(res.data.leaseGeneration); state.leaseToken=String(res.data.leaseToken); state.leaseExpiresAt=Number(res.data.leaseExpiresAt)||0;
      v129WriteProtocolState(state); return {ok:true,state:state};
    }
    function v133ConfirmDestructiveSnapshot(data,sourceName) {
      data=data||{};
      var count=Math.max(0,Number(data.deleteCount)||0), before=Math.max(0,Number(data.beforeCount)||0);
      var ratio=Math.max(0,Number(data.deleteRatio)||0), percent=(ratio*100).toFixed(1);
      var message='安全确认：本次同步将从来源【'+String(sourceName||'当前来源')+'】删除 '+count+' 条联系人。\n\n删除前：'+before+' 条\n删除比例：'+percent+'%\n\n这可能由您刚才的删除操作触发；如果联系人异常消失，请选择“取消”并先检查本地数据。\n\n确认继续删除吗？';
      try {
        var w=(managePanelWindow && !managePanelWindow.closed) ? managePanelWindow : window;
        return !!w.confirm(message);
      } catch(e) { try { return !!confirm(message); } catch(_e) { return false; } }
    }
    function v133ConflictText(res) {
      var d=(res&&res.data)||{}, code=String(d.code||d.reason||'HTTP_409');
      if (code==='DELETE_CONFIRMATION_CANCELLED') return '已取消危险删除；上传已暂停，数据未从服务端删除';
      if (code==='SOURCE_LEASE_HELD') return '该来源正由另一浏览器实例持有，请关闭重复实例后重试';
      if (code==='STALE_WRITER_FENCED') return '写入租约已失效，请刷新页面重新建立同步';
      if (code==='CLIENT_EPOCH_MISMATCH') return '同步世代冲突；为保护数据已暂停，请检查是否复制了浏览器配置';
      if (code==='SNAPSHOT_SEQUENCE_GAP'||code==='SNAPSHOT_SEQUENCE_CONFLICT') return '快照序号冲突；为保护数据已暂停，请执行同步状态校准';
      if (code==='BATCH_CONTENT_MISMATCH') return '上传批次内容冲突；为保护数据已暂停';
      return String(d.message||d.reason||d.error||(res&&res.error)||('HTTP '+((res&&res.status)||409)));
    }
    async function v129UploadSnapshot(sourceId,sourceName,snapshot) {
      ...
      var useV133=state.protocol==='v133', lease=null;
      if (useV133) {
        lease=await v133ClaimLease(sourceId,state);
        if (!lease || !lease.ok) { state.lastError='lease-claim-failed'; v129WriteProtocolState(state); return lease; }
        state=v129ReadProtocolState(sourceId);
      }
      ...
      v129WriteProtocolState(state);
      var requestBody={sourceId:sourceId,sourceName:sourceName,clientEpoch:state.clientEpoch,batchId:batchId,
        snapshotSeq:seq,uploadedAt:Date.now(),itemCount:snapshot.items.length,items:snapshot.items};
      if (useV133) { requestBody.protocolVersion=133; requestBody.installationId=state.installationId; requestBody.leaseGeneration=state.leaseGeneration; requestBody.leaseToken=state.leaseToken; }
      var uploadPath=useV133?'/api/v133/sync/snapshot':'/api/v129/sync/snapshot';
      var res=await localSyncRequest('POST',uploadPath,requestBody);
      // v134：危险删除挑战必须以完全相同的 Snapshot/Batch/Seq 二次提交；令牌仅附加到原请求。
      if (res && res.status===409 && res.data && (res.data.code==='DELETE_CONFIRMATION_REQUIRED' || res.data.reason==='destructive-snapshot-requires-confirmation')) {
        var challenge=res.data, token=String(challenge.confirmationToken||'');
        v128EventLog('delete-confirmation-required',{sourceId:sourceId,batchId:batchId,snapshotSeq:seq,beforeCount:challenge.beforeCount,deleteCount:challenge.deleteCount,deleteRatio:challenge.deleteRatio},'warn');
        if (token && v133ConfirmDestructiveSnapshot(challenge,sourceName)) {
          if (useV133) {
            var renewed=await v133ClaimLease(sourceId,v129ReadProtocolState(sourceId));
            if (!renewed || !renewed.ok) return renewed || {ok:false,status:409,error:'删除确认后租约续期失败'};
            var renewedState=v129ReadProtocolState(sourceId);
            requestBody.installationId=renewedState.installationId;
            requestBody.leaseGeneration=renewedState.leaseGeneration;
            requestBody.leaseToken=renewedState.leaseToken;
          }
          requestBody.destructiveConfirmationToken=token;
          res=await localSyncRequest('POST',uploadPath,requestBody);
          try { delete requestBody.destructiveConfirmationToken; } catch(e0) {}
        } else {
          res={ok:false,status:409,error:'危险删除已取消',data:{ok:false,conflict:true,error:'conflict',code:'DELETE_CONFIRMATION_CANCELLED',reason:'delete-confirmation-cancelled',retryable:false,action:'review_local_data'}};
        }
      }
      state=v129ReadProtocolState(sourceId);
      if (!res.ok) {
        state.lastError=v133ConflictText(res);
        // 只有真正的协议状态冲突才锁定 protocol；用户取消删除不破坏已校准协议。
        var conflictCode=String((res.data&&(res.data.code||res.data.reason))||'');
        if (res.status===409 && conflictCode!=='DELETE_CONFIRMATION_CANCELLED' && conflictCode!=='DELETE_CONFIRMATION_REQUIRED') state.protocol='conflict';
        v129WriteProtocolState(state);
        v128EventLog('snapshot-upload-rejected',{status:res.status,code:conflictCode,reason:state.lastError,batchId:batchId,snapshotSeq:seq,serverAckSeq:res.data&&res.data.serverAckSeq},'warn');
        res.error=state.lastError;
        return res;
      }
      ...
      state.activeBatch=null; state.lastError=''; state.protocol=useV133?'v133':'v129'; v129WriteProtocolState(state);
    ...
    async function localSyncUploadAll() {
      ...
      LOCAL_SYNC_LAST_STATUS.message=res&&res.ok ? '已同步' : ((res&&res.status===409)?v133ConflictText(res):'同步失败');
      return res;
    }
    ...
    function v117Channel() {
      if (STANDALONE_DASHBOARD_MODE) return 'dashboard';
    ...
    function v117BackgroundRender() {
      ...
      if (STANDALONE_DASHBOARD_MODE) return;
      try {
    ...
    function v117StartCoordinator() {
      ...
      v117ApplySharedRuntime(); v117ApplySharedAggregate(false); v117CoordinatorRound();
    }
    ...
    runLinkScan();
    // v130：独立 Dashboard 直接复用当前顶层页面；渠道模式仍沿用原弹窗管理面板。
    var win = STANDALONE_DASHBOARD_MODE ? window : window.open('', 'wa_remark_manage_panel');
    if (!win) { alert('\u65b0\u6807\u7b7e\u9875\u88ab\u62e6\u622a\uff0c\u8bf7\u5141\u8bb8\u672c\u7ad9\u5f39\u51fa\u65b0\u9875\u9762\u540e\u91cd\u8bd5\u3002'); return; }
    managePanelWindow = win;
    win.document.open();
    win.document.write(buildManagePanelShell());
    win.document.close();
    if (STANDALONE_DASHBOARD_MODE) {
      try {
        win.document.documentElement.setAttribute('data-wa-dashboard-mode', 'standalone');
        win.document.title = '聚宝盆客户信息管理系统 v130 · 独立 Dashboard';
      } catch(e) {}
    }
    ...
    function deleteRow(id) {
      if (!id) return;
      // v131：他源记录删除 = 服务端覆盖层墓碑（不影响来源浏览器本地数据，快照重传不复活）。
      if (isRemoteSyncWrappedId(id)) {
        if (!isRemoteEditableEnabled()) { try { win.alert('🔒 这是其他源汇总数据，且当前未开启“他源可编辑”。'); } catch(e) {} return; }
        remoteOverrideDelete(id);
    ...
    function renderTable() {
      ...
      rows.forEach(function (row, idx) {
        ...
        tr.className = 'row-' + row.__platform;
          if (isRemoteSyncRecord(row) && !isRemoteEditableEnabled()) { tr.classList.add('wa-remote-readonly-row'); tr.title = getRemoteSyncReadonlyTitle(row); delete state.selected[row.__id]; }
        if (row.__mainKey) {
          tr.setAttribute('data-main-key', row.__mainKey);
        }
        COLUMNS.forEach(function (col) {
          var td = win.document.createElement('td');
          if (isRemoteSyncRecord(row) && !isRemoteEditableEnabled()) {
            ...
            if (isRemoteSyncRecord(row)) appendRemoteEditableBadge(win.document, sourceBox, row);
            else appendLocalEditableBadge(win.document, sourceBox);
            ...
            if (isRemoteSyncRecord(row) && !isRemoteEditableEnabled()) { sel.disabled = true; sel.title = getRemoteSyncReadonlyTitle(row); sel.style.cursor = 'not-allowed'; sel.style.opacity = '0.75'; }
    ...
    // v131：他源可编辑开关（默认开启）。改动写入服务端覆盖层；关闭后恢复他源只读。
    var remoteEditToggle = win.document.getElementById('remoteEditToggle');
    if (remoteEditToggle) {
      remoteEditToggle.checked = isRemoteEditableEnabled();
      remoteEditToggle.addEventListener('change', function () {
        setRemoteEditableEnabled(remoteEditToggle.checked);
        try {
          if (remoteEditToggle.checked) win.alert('已开启“他源可编辑”：他源行可编辑/删除，改动保存在服务端覆盖层，来源浏览器快照不会覆盖。');
          else win.alert('已关闭“他源可编辑”：他源行恢复只读。');
        } catch (e) {}
        renderAll();
      });
    }
    ...
    win.document.getElementById('batchDeleteBtn').addEventListener('click', function () {
      ...
      var localIds = ids.filter(function (id) { return !isRemoteSyncWrappedId(id); });
      var remoteIds = ids.filter(function (id) { return isRemoteSyncWrappedId(id); });
      if (!localIds.length && !remoteIds.length) return;
      if (remoteIds.length && !isRemoteEditableEnabled()) { win.alert('选中的包含其他源数据，且当前未开启“他源可编辑”。'); return; }
      var msg = '确定删除选中的 ' + ids.length + ' 条记录？';
      if (localIds.length) msg += '\n\n' + localIds.length + ' 条本源记录：本地 + 服务端同步删除。';
      if (remoteIds.length) msg += '\n\n' + remoteIds.length + ' 条他源记录：写入服务端覆盖层墓碑（从汇总消失，不影响来源浏览器本地数据）。';
      if (!win.confirm(msg)) return;
      localIds.forEach(function (id) { deleteRow(id); delete state.selected[id]; });
      remoteIds.forEach(function (id) { remoteOverrideDelete(id); delete state.selected[id]; });
      renderAll();
    });
  ...
  function registerMenuCommands() {
    try {
      ...
      if (typeof GM_registerMenuCommand === 'function') {
        ...
        // v132：多机共用——同步服务地址 / 访问令牌 / 下行指令开关
        GM_registerMenuCommand('设置同步服务地址（多机共用）', function () {
          var cur = '';
          try { cur = getLocalSyncServerUrl(); } catch (e) {}
          var v = prompt('输入中心服务端地址，例如 http://192.168.1.10:8765\n（留空恢复默认 http://127.0.0.1:8765）', cur);
          if (v === null) return;
          try { setLocalSyncServerUrl(v); } catch (e) {}
          alert('已设置同步服务地址：' + getLocalSyncServerUrl() + '\n若服务端设置了令牌，请再用“设置访问令牌”菜单配置。');
        });
        GM_registerMenuCommand('设置访问令牌（可选）', function () {
          var cur = '';
          try { cur = String(GM_getValue('wa_local_sync_token', '') || ''); } catch (e) {}
          var v = prompt('输入服务端 JUBO_TOKEN（留空表示不设令牌）', cur);
          if (v === null) return;
          try { GM_setValue('wa_local_sync_token', String(v || '').trim()); } catch (e) {}
          alert('访问令牌已更新。');
        });
        GM_registerMenuCommand('他源改动下发到来源浏览器（下行指令）', function () {
          var next = !isRemoteDownlinkEnabled();
          setRemoteDownlinkEnabled(next);
          alert('下行指令已' + (next ? '开启' : '关闭') + '。\n开启时，Dashboard 对他源记录的编辑/删除会下发指令，让来源浏览器本地数据收敛到一致；关闭则仅保留服务端覆盖层视图。');
        });
      }
    } catch (e) { }
  }
  ...
  function boot() {
    ...
    // v130：Dashboard 仅启动管理面板、共享配置和同步协调器。
    // 明确跳过 IG 被动监听、History Hook、渠道 DOM Observer、聊天轮询和备注栏挂载。
    if (STANDALONE_DASHBOARD_MODE) {
      try { openManagePanel(); } catch(e) { console.error('[v130 dashboard panel]', e); }
  ...
  /*
  ===============================================================================
  v130 独立 Dashboard 更新说明
  ===============================================================================
  1. 新增 http://127.0.0.1:8765/dashboard 独立入口，直接复用完整管理面板。
  2. Dashboard 模式不执行渠道识别、聊天 Header、备注栏、History Hook 或 DOM Observer。
  3. 继续复用 GM_storage、Leader 选举、原子快照、聚合缓存与自动刷新协议。
  4. 当前 Profile/sourceId 的原始事实记录可编辑；其他来源记录保持严格静态只读。
  5. 跨源/跨渠道 MAIN 派生记录继续只读，聚合结果不会覆盖本地事实库。
  6. 不增加远程命令、WebSocket、长轮询或第二套前端。
  ===============================================================================
  */
...
/*
===============================================================================
V134 正式修复版更新说明（2026-09-15）
===============================================================================
1. 危险删除采用“服务端挑战—用户确认—刷新 Lease—原请求二次提交”的完整闭环。
2. 二次提交保持同一 Snapshot 内容、Snapshot Hash、Batch ID 与 Snapshot 序号，仅附加确认令牌并更新 Lease 凭据。
3. 409 及不可恢复的 4xx 冲突进入 blocked 状态，停止无限自动重试；网络和 5xx 临时故障继续退避重试。
4. 日志补充错误代码、来源、批次、序号和删除统计，并对 Lease Token、认证 Token 做脱敏处理。
5. 保留 /api/v133、protocolVersion=133、v133 内部函数和历史 GM 存储键，以兼容既有服务及同步状态。
6. 本文件须与 V134_中心同步服务.py、V134_shared_db.py 成套部署；升级前请停止服务并备份数据库。
===============================================================================
*/
