...
VERSION="200.0.0"; WORKSPACE="default"
...
def apply_contact(c,e):
    ...
    if op=="contact.observe":
        ...
        obs={k:p.get(k) for k in OBSERVED if k in p}; oh=hash_obj(obs); t=now_ms()
        if not row:
            cid=uid("ct"); c.execute("INSERT INTO contacts(contact_id,workspace_id,source_id,channel,external_contact_id,display_name,observed_phone,avatar_url,created_at,updated_at,last_seen_at,observed_json,observation_hash) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",(cid,WORKSPACE,e["sourceId"],p["channel"],str(p["externalContactId"]),str(p.get("displayName") or ""),str(p.get("observedPhone") or ""),str(p.get("avatarUrl") or ""),t,t,t,dumps(p.get("observed") or {}),oh)); version=1; action="create"
        else:
            cid=row["contact_id"]
            ...
            version=row["version"]+1; c.execute("UPDATE contacts SET display_name=?,observed_phone=?,avatar_url=?,observed_json=?,observation_hash=?,version=?,updated_at=?,last_seen_at=? WHERE contact_id=?",(str(p.get("displayName") or ""),str(p.get("observedPhone") or ""),str(p.get("avatarUrl") or ""),dumps(p.get("observed") or {}),oh,version,t,t,cid)); action="observe"
    ...
    if op=="contact.patch":
        ...
        if not sets:return event_error("EMPTY_PATCH","没有可修改的人工字段")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def route(self):
        ...
        if path=="/" and method=="GET":return self.send_bytes((BASE/"dashboard.html").read_bytes(),"text/html; charset=utf-8")
