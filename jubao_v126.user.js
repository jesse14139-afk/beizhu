// ==UserScript==
// @name         v125、聚宝盆客户信息管理系统、标签画像字段阶段公共配置同步版
// @namespace    wa-remark-helper
// @version      126
// @description  v125：统一标签/画像命名，并跨来源同步标签、画像、字段名称和阶段名称；保留 v124 全部联系人、导出、删除同步与版本徽标功能。
...
(function () {
  ...
  var LOCAL_SYNC_UPLOAD_DIRTY = false;
  var LOCAL_SYNC_UPLOAD_CHANGE_SEQ = 0;
...
function getPanelDisplayData() {
    ...
    function scheduleLocalSyncAutoUpload(reason, preserveDirty) {
      ...
      if (!preserveDirty) {
        LOCAL_SYNC_UPLOAD_DIRTY = true;
        LOCAL_SYNC_UPLOAD_CHANGE_SEQ++;
      }
    ...
    async function drainLocalSyncUploadQueue(reason) {
      if (!isLocalSyncRealtimeUploadEnabled()) return;
      if (LOCAL_SYNC_RUNNING) { LOCAL_SYNC_UPLOAD_DIRTY = true; return; }
      if (!LOCAL_SYNC_UPLOAD_DIRTY && reason !== 'initial-sync') return;
      var uploadLock = await v117AcquireUploadLease();
      if (!uploadLock) {
        LOCAL_SYNC_UPLOAD_DIRTY = true;
        clearTimeout(LOCAL_SYNC_UPLOAD_TIMER);
        LOCAL_SYNC_UPLOAD_TIMER = setTimeout(function(){ drainLocalSyncUploadQueue('upload-lease-retry'); }, 700 + Math.floor(Math.random() * 900));
        return;
      }

      LOCAL_SYNC_RUNNING = true;
      var uploadSeq = LOCAL_SYNC_UPLOAD_CHANGE_SEQ;
      try {
        v892SetRuntime({ upload:'busy', uploadError:'' });
        var h = await localSyncHealthCheck({ silent:true });
        if (!h.ok) {
          LOCAL_SYNC_UPLOAD_DIRTY = true;
          v892SetRuntime({ upload:'retry', uploadError:'同步服务已断开，等待恢复后重试' });
          return;
        }
        var result = await localSyncUploadAll();
        if (!result || !result.ok) {
          LOCAL_SYNC_UPLOAD_DIRTY = true;
          v892SetRuntime({ upload:'error', uploadError:'上传失败：' + ((result && (result.error || result.status)) || '未知错误') });
          return;
        }
        if (LOCAL_SYNC_UPLOAD_CHANGE_SEQ === uploadSeq) LOCAL_SYNC_UPLOAD_DIRTY = false;
        v892SetRuntime({
          upload: LOCAL_SYNC_UPLOAD_DIRTY ? 'pending' : 'ok',
        ...
        LOCAL_SYNC_UPLOAD_DIRTY = true;
        v892SetRuntime({ upload:'error', uploadError:'上传异常：' + ((e && e.message) || e) });
      } finally {
        LOCAL_SYNC_RUNNING = false;
        LOCAL_SYNC_LAST_AUTO_AT = Date.now();
        if (LOCAL_SYNC_UPLOAD_DIRTY && isLocalSyncRealtimeUploadEnabled() && v892SyncRuntime().service === 'ok') {
          scheduleLocalSyncAutoUpload('dirty-after-upload', true);
        }
      }
    }
      ...
      async function heartbeat() {
        ...
        try {
          ...
          if (result.ok && previous === 'error') {
            v117PublishRecovery();
            if (isLocalSyncRealtimeUploadEnabled() && LOCAL_SYNC_UPLOAD_DIRTY) scheduleLocalSyncAutoUpload('service-recovered', true);
            if (isLocalSyncAggregateEnabled()) runLocalSyncAggregateOnce(renderAllFn);
          } else if (!result.ok) {
            if (isLocalSyncRealtimeUploadEnabled() && LOCAL_SYNC_UPLOAD_DIRTY) v892SetRuntime({ upload:'retry' });
    ...
    function v117BecomeLeader() {
      ...
      if (isLocalSyncRealtimeUploadEnabled()) scheduleLocalSyncAutoUpload('initial-sync', true);
      v117PublishRuntimePatch({});
    ...
    function v117ApplySharedRuntime(raw) {
      ...
      if (g.service === 'ok' && LOCAL_SYNC_UPLOAD_DIRTY && isLocalSyncRealtimeUploadEnabled()) scheduleLocalSyncAutoUpload('shared-service-recovered', true);
    }
    ...
    function v117Wake() {
      if (V117_IS_LEADER) { v117RenewLeader(); return; }
    ...
    function v117StartCoordinator() {
      ...
      v117RegisterListener(V117_RECOVERY_KEY,function(n,o,v,remote){ if(remote && LOCAL_SYNC_UPLOAD_DIRTY && isLocalSyncRealtimeUploadEnabled()) scheduleLocalSyncAutoUpload('service-recovered-broadcast',true); });
      ...
      if (upToggle) {
        upToggle.onchange = async function() {
          ...
          if (on) {
            ...
            clearTimeout(LOCAL_SYNC_UPLOAD_TIMER); v892SetRuntime({ upload:'off', uploadError:'' });
          }
