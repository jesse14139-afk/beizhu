// ==UserScript==
// @name         聚宝盆客户信息管理系统 v134 独立Dashboard
...
function getPanelDisplayData() {
    ...
    function remoteOverrideDelete(channelId) {
      ...
      var parts = v99ParseRemoteKey(channelId);
      if (!parts) return false;
      // v133：删除墓碑/覆盖层与 contact.delete 命令原子提交。
      localSyncRequest('POST', '/api/v133/admin/contacts/delete', { sourceId: parts.sid, contactId: parts.cid, updatedBy: getLocalSyncSourceName() }).then(function (res) {
        if (res && res.ok && res.data && res.data.ok === true) {
          try { delete state.selected[channelId]; } catch (e) {} remoteOverrideRefresh();
        } else remoteOverrideToast('⚠️ 他源删除失败：' + remoteOverrideFailText(res));
      });
      return true;
    }
